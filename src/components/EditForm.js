...
function EditForm() {
    ...
    useEffect(() => {
        const fetchFormAndTemplate = async () => {
            ...
            try {
                ...
                if (!formRes.ok) throw new Error("Failed to fetch form for editing");
                const formData = await formRes.json();
                setFormRecord(formData);

                // 2) Fetch template (Questions)
                const templateRes = await fetch(
                    `${API_URL}/api/templates/${formData.template_id}`,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
                if (!templateRes.ok) throw new Error("Failed to fetch template for form");
    ...
    const sortedQuestions = useMemo(() => {
        ...
        // Stable ordering if backend doesn't guarantee order
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Convert answers object => [{ question_id, answer_value }]
            ...
            if (!response.ok) {
                // If backend returns JSON error, try to show it
    ...
    if (!formRecord || !template) {
        // Shouldn’t happen often, but keeps UI sane
        return (
            <div className="mx-auto w-full max-w-3xl px-4 py-10">
                <Card className="p-6">
                    <p className="text-sm text-zinc-600 dark:text-zinc-400">
                        Missing form or template data.
                    </p>
                    <div className="mt-4">
                        <Button variant="outline" onClick={() => navigate(-1)} className="gap-2">
                            <ArrowLeft className="h-4 w-4" />
                    ...
                    {sortedQuestions.length > 0 ? (
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            {sortedQuestions.map((q) => {
                                const currentVal = answers[q.id] ?? (q.question_type === "checkbox" ? "false" : "");
                                        ...
                                        {q.question_type === "checkbox" && (
                                            <label className="flex items-center gap-2 rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950">
                                                <input
                                                    ...
                                                    onChange={(e) => handleAnswerChange(q.id, q.question_type, e.target.checked)}
                                                    className="h-4 w-4"
                                                />
                                                <span className="text-zinc-700 dark:text-zinc-300">Check if true</span>
                                            </label>
                                        )}
                                    </div>
                                );
